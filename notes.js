...
class NotesManager {
  ...
  positionAndShowMenu(menu, editor) {
    // 获取编辑器工具栏的位置 - EasyMDE 使用 editor-toolbar 类
    let toolbar = null;
    if (editor && editor.element) {
      // 从编辑器元素向上查找 EasyMDEContainer
  ...
  bindEvents() {
    ...
    // 笔记标签输入
    document.getElementById('edit-note-tags').addEventListener('keydown', (e) => {
      if (e.key === 'Enter') {
        e.preventDefault();
    ...
    // 提示词标签输入
    document.getElementById('edit-prompt-tags').addEventListener('keydown', (e) => {
      if (e.key === 'Enter') {
        e.preventDefault();
        const tag = e.target.value.trim();
        if (tag) {
          this.addPromptTag(tag);
          e.target.value = '';
        }
      }
    });
  ...
  initNoteEditor(content) {
    ...
    this.noteEditor = new EasyMDE({
      ...
      placeholder: '输入笔记内容，支持 Markdown 和 HTML 格式，使用 AI 工具栏可智能编辑',
      ...
      toolbar: [
        ...
        { name: 'side-by-side', action: EasyMDE.toggleSideBySide, className: 'fa fa-columns no-disable no-mobile toolbar-btn-text', title: '分屏' },
        ...
        { name: 'ai-menu', action: (editor) => this.showAIMenu(editor), className: 'fa fa-ellipsis-h toolbar-btn-text', title: 'AI工具' }
      ],
      minHeight: '200px',
      maxHeight: '400px',
      renderingConfig: {
  ...
  async saveNote() {
    ...
    const content = this.noteEditor ? this.noteEditor.value().trim() : '';
    ...
    if (!title) {
      title = this.buildAutoTitle(content, this.currentTags, this.editingImages);
  ...
  renderPromptEditTags() {
    const container = document.getElementById('edit-prompt-tags-list');
    container.innerHTML = this.promptCurrentTags.map(tag => `
      <span class="tag-item">
        ${tag}
        <span class="remove-tag" data-tag="${tag}">×</span>
      </span>
  ...
  async savePrompt() {
    ...
    const content = this.promptEditor ? this.promptEditor.value().trim() : '';
  ...
  renderBlockedInputsManager() {
    ...
    hostSelect.innerHTML = `
      ...
      ${hosts.map(host => `<option value="${host}">${host} (${blockedInputs[host].length})</option>`).join('')}
    `;
